...
class UPEMD:
    ...
    def make_modes(self):
        self.imf = np.zeros((self.last_mode,self.n_data))
        ...
        for mode in range(self.start_mode,self.last_mode - 1):
            ...
            self.imf[self.count_mode,:] = self.sum_wrk/self.count_shift

            self.res = self.res - self.imf[self.count_mode,:]

            self.count_mode += 1
        self.imf[self.count_mode,:] = self.res
