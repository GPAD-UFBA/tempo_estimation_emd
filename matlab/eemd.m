...
% fast EMD/EEMD/CEEMD code: VERSION 1.1 2018/0724
%  Copyright (C) RCADA, National Central University; July, 2018 
% Notice: What are the Differences in this Version ?
% (1) Have Modifed the eemd code so that the code can be executted in different versions of matlabs
% (2) Have Fixed a Bug in emd when typeSpline=2
...
% Usage:
% (A) run EMD
% [imf] = emd(Y, toModifyBC, typeSpline, numImf, maxSift);
% (B) run EEMD
% (i) [allmode]=eemd(Y, NoiseLevel, NE, numImf)
% (ii) [allmode]=eemd(Y,NoiseLevel,NE,numImf,runCEEMD,maxSift,typeSpline,toModifyBC,randType,seedNo,checkInput);
...
% typeSpline: default=2; 1: clamped spline; 2: not a knot spline; 3:natural cubic spline; sometimes  natual cubic spline will be better
...
%
% Modified Notes
% 2018-3-23, matlab vesion after R2013a, replace RandStream.getDefaultStream with RandStream.getGlobalStream in the functions set_seed and return_seed
...
function savedState = org_set_seed(seedNo)
% 2018-0415
...
%savedState = defaultStream.State;
% modify 2018-0723
if ismethod('RandStream', 'getDefaultStream') % YHW2018-0615
    defaultStream = RandStream.getDefaultStream;
    savedState = defaultStream.State;   
elseif ismethod('RandStream', 'getGlobalStream') % After release R2013a
    defaultStream = RandStream.getGlobalStream;
    savedState = defaultStream.State;
else
    fprintf('ERROR : Another incompatible change by the Mathworks...\n');
end
